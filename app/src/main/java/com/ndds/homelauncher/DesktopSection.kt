...
class DesktopSection(val appContext: MainActivity) {
    ...
    fun toggleEditState() {
        val v = rootView.findViewById<View>(R.id.action_edit_fav_list)
        if (adapter.isEditMode) {
            v.background = AppCompatResources.getDrawable(appContext, R.drawable.edit)
            pinnedAppList = ArrayList(adapter.appList.map { it.id })
            StorageService(appContext).updatePinnedApps(adapter.appList)
        } else
            v.background = AppCompatResources.getDrawable(appContext, R.drawable.check)
